...
/* 陈爱军在2023年3月28日添加了以下矩阵操作，经过测试应该没什么问题 */
// 刚发现矩阵数乘运算原工具箱中已经自带了..
Matrix *Matrix_opposite(Matrix * _mat); //取相反数
Matrix * M_cat(Matrix *_mat1, Matrix *_mat2, const int DIM);   //矩阵拼接
Matrix * M_scalarMul(Matrix *_mat1, const MATRIX_TYPE scalar); // 矩阵数乘

void reduceG(TYPE*, TYPE*, const int, const int, const int);
TYPE computeVectorNorm(TYPE*, int);
TYPE computeVectorSum(TYPE*, int);
...
TYPE M_cond(Matrix *_mat, int Setting);
