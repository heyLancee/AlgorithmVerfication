...
TYPE M_cond(Matrix *_mat, int Setting){ /* condition number
    矩阵条件数 */
    int i,j;
    TYPE matrix_cond = 0;
...
Matrix *Matrix_opposite(Matrix * _mat){
    /* 计算矩阵的相反数 */
    Matrix *_mat_result = NULL;
    _mat_result = Matrix_copy(_mat);
    int size = (_mat_result->row) * (_mat_result->column), i;
    for (i = 0; i < size; i++) {
        _mat_result->data[i] = -1.0 * _mat_result->data[i];
    }
    return _mat_result;
}

Matrix * M_cat(Matrix *_mat1, Matrix *_mat2, const int DIM){
    Matrix *_mat_result = NULL;
    if (DIM == 1){ // 行向拼接
        if (_mat1->column != _mat2->column){
            printf(M_mul_001);
        } else {
            _mat_result = (Matrix *) malloc(sizeof(Matrix));
            int row = _mat1->row + _mat2->row;  // 行向拼接结果的行数
            int column = _mat2->column;         // 行向拼接结果的列数
            MATRIX_TYPE *_data = (MATRIX_TYPE *) malloc((row * column) * sizeof(MATRIX_TYPE));
            int i, j;

            for (i = 0; i < _mat1->row; i++) {
                for (j = 0; j < column; j++) {
                    _data[i * column + j] = _mat1->data[i * column + j];
                }
            }
            for (i = 0; i < _mat2->row; i++) {
                for (j = 0; j < column; j++) {
                    _data[i * column + j + _mat1->row * column] = _mat2->data[i * column + j];
                }
            }
            _mat_result->row = row;
        ...
        if (_mat1->row != _mat2->row){
            printf(M_mul_001);
        } else {
            _mat_result = (Matrix *) malloc(sizeof(Matrix));
            int row = _mat1->row;                       // 列向拼接结果的行数
            int column = _mat1->column + _mat2->column; // 列向拼接结果的列数
            MATRIX_TYPE *_data = (MATRIX_TYPE *) malloc((row * column) * sizeof(MATRIX_TYPE));
            int i, j;
            for (i = 0; i < row; i++) {
                for (j = 0; j < column; j++) {
                    if ( j < _mat1->column ){
                        _data[i * column + j] = _mat1->data[i * _mat1->column + j];
                    } else {
                        _data[i * column + j] = _mat2->data[i * _mat2->column + (j-_mat1->column)];
                    }
                }
            }
            _mat_result->row = row;
...
Matrix * M_scalarMul(Matrix *_mat1, const MATRIX_TYPE scalar){
    // 矩阵数乘运算 A * dt
    Matrix * _mat_result = Matrix_copy(_mat1);
    int row = _mat_result->row;
    int column = _mat_result->column;
    int i, j;
    for (i = 0; i < row; i++) {
        for (j = 0; j < column; j++) {
            _mat_result->data[i * column + j] = scalar * _mat_result->data[i * column + j];
        }
    }
    return _mat_result;
}
...
void reduceG(TYPE* InputArray, TYPE* OutputArray, const int row, const int col, const int ngen) {
    /*  功能：生成矩阵降阶
    输入：
        InputArray: 需要降阶的矩阵
        row: InputArray 的行数
    ...
    */
    ...
    if (col <= ngen) {
        ...
        for (i = 0; i < row; i++) {
            for (j = 0; j < ngen; j++) {
                if (j < col) {
                    OutputArray[i * ngen + j] = InputArray[i * col + j];
                }
                else {
                    OutputArray[i * ngen + j] = 0;
                }
            }
        }
    }
        ...
        // 计算每一列向量的范数
        TYPE* normGvec = (TYPE*)malloc(sizeof(TYPE) * col);
        TYPE* tempCol = (TYPE*)malloc(sizeof(TYPE) * row);// 临时列向量

        for (i = 0; i < col; i++) {
            for (j = 0; j < row; j++) {
                tempCol[j] = InputArray[j * col + i];
            }
            normGvec[i] = computeVectorNorm(tempCol, row);
        }

        int* index = (int*)malloc(sizeof(int) * col);
        BubbleSort(normGvec, index, col, 1);

        //将Gk的生成矩阵，按照范数大小降序排列
        TYPE* Greduced = (TYPE*)malloc(sizeof(TYPE) * row * col);

        for (i = 0; i < row; i++) {
            for (j = 0; j < col; j++) {
                Greduced[i * col + j] = InputArray[i * col + index[j]];//没想通
            }
        }


        TYPE* temp1 = (TYPE*)malloc(sizeof(TYPE) * row * (ngen - row));
        for (i = 0; i < row; i++) {
            for (j = 0; j < ngen - row; j++) {
                temp1[i * (ngen - row) + j] = Greduced[i * col + j];
            }
        }

        TYPE* temp2 = (TYPE*)malloc(sizeof(TYPE) * row * (col - ngen + row));

        for (i = 0; i < row; i++) {
            for (j = 0; j < col - ngen + row; j++) {
                temp2[i * (col - ngen + row) + j] = Greduced[i * col + j + ngen - row];
            }
        }

        TYPE* temp3 = (TYPE*)malloc(sizeof(TYPE) * row * row);
        memset(temp3, 0, sizeof(TYPE) * row * row);
        for (i = 0; i < row; i++) {
            temp3[i * row + i] = computeVectorSum(temp2 + i * (col - ngen + row), col - ngen + row); // temp2指针
        }
        ...
        for (i = 0; i < row; i++) {
            for (j = 0; j < ngen; j++) {
                if (j < ngen - row) {
                    OutputArray[i * ngen + j] = temp1[i * (ngen - row) + j];
                }
                else {
                    OutputArray[i * ngen + j] = temp3[i * row + j - ngen + row];
                }
            }
        }
...
TYPE computeVectorNorm(TYPE* InputArray, int row) {
    TYPE vnorm2 = 0;

    int i = 0;
    for (i = 0; i < row; i++) {
        vnorm2 += *(InputArray + i) * *(InputArray + i);
    }

    return sqrt(vnorm2);
}

TYPE computeVectorSum(TYPE* InputArray, int row) {
    TYPE vSum = 0;
...
*Name            :BubbleSort
*Fun             :冒泡排序，并返回索引
*InputParaments  :data是待排序的数据;index是输出参数，用于返回索引;
                  len是数组长度;mode是排序模式,0代表升序，1代表降序。
*OutputParaments :None
****************************************************************/
void BubbleSort(TYPE* data, int* index, int len, int mode) {
    //索引初始化
    for (int i = 0; i < len; i++) {
        index[i] = i;
    }
...
Matrix* M_reduceG(Matrix* Mat, int ngen) {
    int row = Mat->row;         // 矩阵行数
    int col = Mat->column;      // 降维前列数

    Matrix* _mat_result = NULL;
    _mat_result = (Matrix*)malloc(sizeof(Matrix));

    MATRIX_TYPE* _data = (MATRIX_TYPE*)malloc((row * ngen) * sizeof(MATRIX_TYPE));
    reduceG(Mat->data, _data, row, col, ngen);

    _mat_result->row = row;
    _mat_result->column = ngen;
    _mat_result->data = _data;
...
Matrix *M_find(Matrix *_mat, MATRIX_TYPE value) {/*Find position with the value in Matrix
	...
	// define mat_1
		...
		Matrix*  mat_1 = Matrix_gen(row,column,(TYPE *)_mat_1);
	// find mat_1
...
Matrix * M_eigen_val(Matrix * _mat){
    ...
    M_eigen_val->row = 1;
    eigen_val = (TYPE*)malloc(sizeof(TYPE)*dim);
    for(i=0;i<dim;i++){
...
void progress_bar(int count, int total) {
    TYPE num = (int)((1.0*count/total)*50);
    printf("%% %.2f[", num*2);
    for(int i = 0; i < 50; i++){
        (i < num)?printf(">"):printf(" ");
    }
